...
<html>
    ...
    <body class="bg-emerald-600">
                    ...
                    <div class="flex w-full">
                        <div>
                            <div class="flex justify-center items-center">
                                <form action="/cuongproject/admin/seas/edit" style="min-width: 500px" id="form-sea-edit">
                                    <div class="mb-6">
                                    ...
                                    <div class="mb-6 grid grid-cols-3 space-3">
                                        <c:forEach items="${sea.images}" var="image">
                                            <div class="mb-4" id="image-item-${image.id}">
                                                <div class="max-w-[150px] bg-white rounded-lg border border-gray-200 shadow-md">
                                                    ...
                                                    <div class="flex flex-col items-center">
                                                        <img class="mb-3 w-full h-full" src="/assets/images/seas/${image.image}" alt="Bonnie image"/>
                                                    </div>
                                                </div>
                                            </div>
                                        </c:forEach>
        ...
        <script>
            ...
            $("#id").val(${sea.id})
            $("#category").val(${sea.category.id}),
            $("#name").val("${sea.name}"),
            $("#brand").val("${sea.brand}"),
            $("#price").val(${sea.price}),
            $("#quantity").val(${sea.quantity}),
            $("#discount").val(${sea.discount}),
            $("#state").val(${sea.state.id}),
            $("#description").val(`${sea.description}`),
            $("#isSale").prop('checked', ${sea.isSale});
    
            new_editor.setData(`${sea.content}`)
           ...
           $("#form-sea-edit").on("submit", function(e){
               e.preventDefault();
               if(imagesDelete.length>0){
                    imagesDelete.forEach((item) => {
                        $.ajax({
                            method: "POST",
                            url: "/cuongproject/admin/seas/image/delete",
                            data: {id: item},
                        }).done(function (data) {
               ...
               console.log(new FormData(document.getElementById("form-sea-edit")));
               $.ajax({
                    method: "POST",
                    url: "/cuongproject/admin/seas/edit",
                    miniType: "multipart/form-data",
                    data: new FormData(document.getElementById("form-sea-edit")),
                    cache: false,
                    contentType: false,
                    processData: false,
                }).done(function (data) {
